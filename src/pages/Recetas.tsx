...
const BORRADOR_KEY = 'rayku-borrador-receta'
...
export default function Recetas({
  ...
  const [requiereDescongelar, setRequiereDescongelar] = useState(false)
  ...
  const [customCaracteristica, setCustomCaracteristica] = useState('')
  ...
  useEffect(() => {
    ...
    try {
      ...
      setRaciones(data.raciones || 1)
      setDificultad(data.dificultad || 'Fácil')
      setRequiereDescongelar(data.requiereDescongelar || false)
      setValoracion(data.valoracion || 0)
      setTiposComida(data.tiposComida || [])
      setIngredientesBase(data.ingredientesBase || [])
      setDietas(data.dietas || [])
      setCaracteristicas(data.caracteristicas || [])

      if (data.nombre || data.ingredientes || data.pasos || data.nota) {
  ...
  useEffect(() => {
    if (!borradorCargado || editandoId) return
  ...
  useEffect(() => {
    ...
    const receta = recetas.find((r) => r.id === recetaSeleccionadaId)
  ...
  }, [recetaSeleccionadaId, recetas, onRecetaSeleccionadaLeida])
  ...
  const cargarRecetaEnFormulario = (receta: Receta) => {
    ...
    setRaciones(receta.raciones)
    setDificultad(receta.dificultad)
    setRequiereDescongelar(receta.requiereDescongelar)
    setValoracion(receta.valoracion)
    setTiposComida(receta.tiposComida)
    setIngredientesBase(receta.ingredientesBase)
    setDietas(receta.dietas)
    setCaracteristicas(receta.caracteristicas)
    setMostrarFormulario(true)
  }
  ...
  const guardarReceta = () => {
    ...
    if (ahora - ultimoGuardadoRef.current < 600) {
      return
    }

    ultimoGuardadoRef.current = ahora

    const nombreLimpio = nombre.trim()

    if (!nombreLimpio) {
      setMensaje('Ponle nombre a la receta 💕')
      return
    }
    ...
    try {
      if (editandoId) {
        updateReceta(editandoId, datosReceta)
      ...
      setMensaje('No se ha podido guardar 💔')
    }
  }
  ...
  return (
    <div>
      ...
      >
        <div>
          ...
          <p style={{ color: '#9e7d90' }}>
            Tu colección cute de comidas favoritas 💕
          </p>
        </div>
      ...
      >
        ...
        >
          <div>
            <h2 style={{ marginBottom: 8 }}>✨ Chef Rayku IA</h2>

            <p style={{ color: '#8f7080' }}>
              Dime ingredientes o qué te apetece y Rayku te ayudará 💕
            </p>
          </div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <h3 style={{ marginBottom: 14 }}>🔎 Buscar y filtrar</h3>
      ...
      {mostrarFormulario && (
        <FormularioReceta
          ...
          requiereDescongelar={requiereDescongelar}
          setRequiereDescongelar={setRequiereDescongelar}
          valoracion={valoracion}
          setValoracion={setValoracion}
          tiposComida={tiposComida}
          setTiposComida={setTiposComida}
          ...
          customDieta={customDieta}
          setCustomDieta={setCustomDieta}
          customCaracteristica={customCaracteristica}
          setCustomCaracteristica={setCustomCaracteristica}
      ...
      <div style={{ display: 'grid', gap: 16 }}>
        ...
        {recetasFiltradas.map((receta) => (
          <CardReceta
            key={receta.id}
            receta={receta}
            onEditar={cargarRecetaEnFormulario}
            onEliminar={deleteReceta}
